defmodule QuickSortTest do
  ...
  describe "sort" do
    ...
    test "returns single element for single element" do
      assert QuickSort.sort([7], &(&1 <= &2)) == [7]
    end

    test "sorts two items" do
      assert QuickSort.sort([7, 3], &(&1 <= &2)) == [3, 7]
    end

    test "sorts 3 items" do
      assert QuickSort.sort([7, 3, 11], &(&1 <= &2)) == [3, 7, 11]
    end

    test "sorts 3 items with duplicates" do
      assert QuickSort.sort([7, 3, 3], &(&1 <= &2)) == [3, 3, 7]
    end

    test "sorts ten items" do
      sorted_list = Enum.to_list(1..10)
      randomized_list = Enum.shuffle(sorted_list)
      assert QuickSort.sort(randomized_list, &(&1 <= &2)) == sorted_list
    end
